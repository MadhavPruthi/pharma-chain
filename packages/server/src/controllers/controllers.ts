...
export async function SupplychainController_getAllTransactions_get(req: Request, res: Response): Promise<void>{
    try{
        let params = req.params;
        res.status(200).send(await SupplychainControllerBackEnd
            .getAllTransactions());
        ...
        console.log('Error get SupplychainController_getAllTransactions', ex.stack);
        res.status(500).send(ex);
    }
}